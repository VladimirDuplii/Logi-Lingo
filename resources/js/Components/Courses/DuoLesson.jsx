...
const ProgressBar = ({ value, total, onExit, hearts }) => {
  ...
  return (
    <header className="flex items-center gap-4">
      <button className="text-gray-400" onClick={onExit} aria-label="Exit lesson">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2" />
        </svg>
      </button>
      <div className="h-4 grow rounded-full bg-gray-200" role="progressbar" aria-valuemin={0} aria-valuemax={1} aria-valuenow={ratio}>
        <div
          className={"h-full rounded-full bg-green-500 transition-all duration-700 " + (value > 0 ? 'px-2 pt-1 ' : '')}
      ...
      >
        <div className="flex max-w-5xl flex-col gap-4 p-5 sm:mx-auto sm:flex-row sm:items-center sm:justify-between sm:p-10 sm:py-14">
          {isAnswerCorrect ? (
            ...
            <div className="mb-2 flex flex-col gap-5 sm:flex-row sm:items-center">
              <div className="hidden rounded-full bg-white p-5 text-red-500 sm:block">✕</div>
        ...
        <div className="flex min-h-[60px] flex-wrap gap-1 border-b-2 border-t-2 border-gray-200 py-1">
          {selectedIndices.map((i) => (
            <button
              ...
              onClick={() =>
                setSelectedIndices((xs) => xs.filter((x) => x !== i))
              }
            >
              {answerTiles[i]}
            </button>
        ...
        <div className="mt-2 flex flex-wrap justify-center gap-1">
          {answerTiles.map((word, i) => (
            <button
              ...
              onClick={() =>
                setSelectedIndices((xs) => (xs.includes(i) ? xs : [...xs, i]))
              }
            >
              {word}
            </button>
...
const DuoLesson = ({ courseId, unitId, lessonId, onExit }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await CourseService.getQuestions(courseId, unitId, lessonId);
  ...
  <ProgressBar value={correctCount + incorrectCount} total={totalNeeded} onExit={onExit} hearts={hearts} />
      ...
      <CheckPanel
        isAnswerSelected={mcMode ? selected !== null : selectedIndices.length > 0}
